...
import { CartService, VendorCart, CartItem } from 'src/app/services/apps/cart/cart.service';
  ...
  template: `
    <div class="cart-dialog-fullscreen">
      <div class="dialog-header">
        <h2 class="dialog-title">{{ data.mode === 'general' ? 'Carrito de Compras' : 'Carrito del Vendedor' }}</h2>
        ...
        @if(data.mode === 'general'){
              ...
              @for(cart of carts; track cart.vendorId) {
                <mat-expansion-panel class="vendor-expansion-panel">
                  <mat-expansion-panel-header>
                    ...
                    <mat-panel-description>{{ cart.items.length }} productos - {{ cart.total | monedaARS }}</mat-panel-description>
                  </mat-expansion-panel-header>
                  <div class="cart-items">
                    @for(item of cart.items; track item.product.id) {
                      <div class="cart-item">
                        <div class="item-top-row">
                          <img [src]="item.product.imagePath" alt="product" class="item-image" />
                          <div class="item-details">
                            <h4>{{ item.product.product_name }}</h4>
                            <p>{{ item.product.base_price | monedaARS }} x {{ item.quantity }}</p>
                            <p class="item-subtotal"><strong>Subtotal: {{ (item.product.base_price * item.quantity) | monedaARS }}</strong></p>
                          </div>
                        </div>
                        <div class="quantity-controls">
                          <button mat-icon-button (click)="updateQuantity(item.product.id, cart.vendorId, item.quantity - 1)">
                          ...
                          <button mat-icon-button color="warn" (click)="removeItem(item.product.id, cart.vendorId)">
                  ...
                  <div class="cart-summary">
                    ...
                    <button mat-flat-button color="primary" class="checkout-btn" (click)="checkoutViaWhatsApp(cart)">
          ...
          @if(selectedCart && selectedCart.items.length > 0){
            <div class="cart-items">
              @for(item of selectedCart.items; track item.product.id) {
                <div class="cart-item">
                  ...
                  <div class="quantity-controls">
                    <button mat-icon-button (click)="updateQuantity(item.product.id, selectedCart.vendorId, item.quantity - 1)">
                      <mat-icon>remove</mat-icon>
                    </button>
                    <span>{{ item.quantity }}</span>
                    <button mat-icon-button (click)="updateQuantity(item.product.id, selectedCart.vendorId, item.quantity + 1)">
                    ...
                    <button mat-icon-button color="warn" (click)="removeItem(item.product.id, selectedCart.vendorId)">
                      <mat-icon>delete</mat-icon>
                    </button>
                  </div>
                </div>
            ...
            <div class="cart-summary">
              ...
              <button mat-flat-button color="primary" class="checkout-btn" (click)="checkoutViaWhatsApp()">
                <mat-icon>whatsapp</mat-icon>
                <span>Finalizar compra</span>
              </button>
            </div>
  ...
  constructor(
    ...
    this.cartService.carts$.subscribe(carts => {
      ...
      if (data.mode === 'vendor' && data.vendorId) {
        this.selectedCart = carts.find(cart => cart.vendorId === data.vendorId) || null;
